                    ...
                    <table id="datatables" class="display table table-striped" style="min-width: 845px">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center"><i class="fa fa-users"></i> {{ $book->guest_amount }}</td>
                                <td class="text-center">{{ $book->sittingArea()->first()->name }}</td>
                                <td class="text-end">{{ Carbon\Carbon::parse($book->reservation_date)->format('Y-m-d') }}</td>
                                <td class="text-end">{{ Carbon\Carbon::parse($book->reservation_time)->locale('id')->format('h:i A'); }}</td>
                                <td>{{ $book->allergy_note }}</td>
                                <td>{{ $book->request_note }}</td>
                                <td>{{ $book->type }}</td>
                                <td>
                                    <a href="{{ '/uploads/proofs/'.$book->proof }}" target="_blank">{{ $book->proof }}</a>
                                </td>
                                <td class="text-end">{{ $book->created_at }}</td>
                                <td class="">
                                    <div class="d-flex gap-2 justify-content-center">
                                        <a href="{{ route('books.show', $book->id) }}" class="btn btn-primary d-flex align-items-center"><i class="fa fa-eye"></i></a>
                                        ...
                                        <a href="{{ route('books.verify', $book->id) }}" class="btn btn-success gap-2"><i class="fa fa-check"></i> Verify</a>
                                        <a href="{{ route('books.reject', $book->id) }}" class="btn btn-warning">
